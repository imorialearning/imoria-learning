...
export default function Home() {
  ...
  useEffect(() => {
    // Client safely initialized inside useEffect to prevent SSR/Build-time variable issues
    const url = process.env.NEXT_PUBLIC_SUPABASE_URL || '';
    const anonKey = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || '';

    if (!url || !anonKey) {
      setErrorMsg("Missing database configuration keys.");
      setLoading(false);
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-[#0D1B2A] text-white font-sans antialiased">
      ...
      <main className="max-w-6xl mx-auto px-4 pb-24">
        ...
        {/* Alert Error Box */}
        {errorMsg && (
          <div className="bg-red-950/40 border border-red-500/30 text-red-200 p-4 rounded-xl mb-8 text-sm max-w-xl mx-auto text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {subjects.length === 0 ? (
              ...
              subjects.map((subject) => (
                <div 
                  key={subject.id} 
                  className="bg-[#1A2E42] border border-[#C9A84C]/10 rounded-xl p-6 transition-all duration-300 hover:border-[#C9A84C]/40 shadow-sm hover:-translate-y-1 group cursor-pointer"
                >
